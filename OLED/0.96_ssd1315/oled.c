 ...
 * 参    数：column 指定光标所在的列坐标，范围：0 ~ 127
 * 返 回 值：无
 * 说    明：OLED默认的X轴，只能8个Bit为一组写入，即1页等于8个X轴坐标
 */
...
/**
 * 函    数：设置全部区域水平滚动
 * 参    数：dir 滚动的方向 水平右滚动：0x26 水平左滚动：0x27
 * 返 回 值：无
 * 说    明：调用此函数后，OLED全部区域水平滚动
 */
...
/**
 * 函    数：设置部分区域水平滚动
 * 参    数：dir 滚动的方向 0x26 水平左滚动：0x27
 * 参    数：start 开始页地址
 * 参    数：end 结束页地址
 * 返 回 值：无
 * 说    明：调用此函数后，OLED部分区域水平滚动
 */
...
/**
 * 函    数：设置水平与垂直滚动滚动
 * 参    数：dir 滚动的方向 垂直和水平右滚动：0x29 垂直和水平左滚动：0x2A垂直和水平右滚动
 * 参    数：start 开始页地址
 * 参    数：end 结束页地址
 * 返 回 值：无
 * 说    明：调用此函数后，OLED部分区域水平滚动
...
/**
 * 函    数：设置显示亮度
 * 参    数：Bright 0x00 ~ 0xff 显示屏的默认亮度值通常是 0x7F
 * 返 回 值：无
 * 说    明：显示屏的默认亮度值通常是 0x7F
 */
 ...
 * 参    数：X 指定数字左上角的横坐标，屏幕区域：0 ~ 127
 * 参    数：Y 指定数字左上角的纵坐标，屏幕区域：0 ~ 63
...
/**
  ...
  * 参    数：X 指定格式化字符串左上角的横坐标，屏幕区域：0 ~ 127
  * 参    数：Y 指定格式化字符串左上角的纵坐标，屏幕区域：0 ~ 63
  * 参    数：FontSize 指定字体大小
  *           范围：OLED_8X16		宽8像素，高16像素
  *                 OLED_6X8		宽6像素，高8像素
